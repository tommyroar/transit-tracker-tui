...
- [ ] 1. Write property tests for existing WebSocket and config behavior
...
- [ ] 3. Create Dockerfile and entrypoint script
  - [ ] 3.1 Create `Dockerfile` at the project root
...
- [ ] 4. Create start and stop lifecycle scripts
  - [ ] 4.1 Create `scripts/start_container.sh`
  ...
  - [ ] 4.2 Create `scripts/stop_container.sh`
...
- [ ] 5. Checkpoint - Verify Dockerfile builds and scripts work
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Create container integration tests
  - [ ] 6.1 Create `tests/test_container.py` with pytest-based container tests
...
- [ ] 7. Extend equivalence test for container-vs-container comparison
  - [ ] 7.1 Extend `scripts/verify_cloud_equivalence.py` for container comparison
...
- [ ] 9. Create Kiro steering file for Docker conventions
  - [ ] 9.1 Create `.kiro/steering/docker.md` with auto-inclusion
...
- [ ] 11. Replace Nomad raw_exec deployment with Docker container for live UAT
  - [ ] 11.1 Update `transit-tracker.nomad.hcl` to use Docker driver instead of `raw_exec`
  ...
  - [ ] 11.2 Stop the current Nomad job and redeploy with the container
