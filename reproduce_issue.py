    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for stop_id, subs in stops_to_poll.items():
            ...
            if response.status_code == 200:
                ...
                # Format trips for the WebSocket-style state
                sim.state[stop_id] = {
                    "trips": entries,
                    "timestamp": time.time()
                }
                ...
                for sub in subs:
                    ...
                    # Filter entries for this sub just for the debug print
                    ...
                    filtered = [e for e in entries if e.get("routeId") == target_route_id or e.get("routeShortName") == target_short_name]
                    
                    for f in filtered:
                        arr = f.get("predictedArrivalTime") or f.get("scheduledArrivalTime")
                        now_ms = int(time.time() * 1000)
                        diff_min = (arr - now_ms) / 60000.0
                        print(f"  Trip {f.get('tripId')}: Arr={arr}, Diff={diff_min:.1f}m")
