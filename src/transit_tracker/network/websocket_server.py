...
class TransitServer:
    ...
    def sync_state(self):
        \"\"\"Updates the internal state for the GUI to monitor.\"\"\"
        if self.config.mock_state is not None:
            return
            
        active_subs = []
        for ws, subs in self.subscriptions.items():
            name = self.client_names.get(ws, f\"{ws.remote_address[0]}:{ws.remote_address[1]}\")
            active_subs.append({
                \"client\": name,
                \"count\": len(subs),
                \"last_seen\": time.time()
            })
        self.config.mock_state = active_subs
    ...
    async def get_arrivals_cached(self, stop_id: str) -> List[Dict[str, Any]]:
        ...
        # Use in-flight tasks to avoid redundant calls for the same stop
        if stop_id in self.in_flight:
            return await self.in_flight[stop_id]

        task = asyncio.create_task(self.api.get_arrivals(stop_id))
        ...
        try:
            async for message in ws:
                ...
                if data.get(\"event\") == \"subscribe\":
                    ...
                    if \"routeStopPairs\" in payload:
                        pairs_str = payload[\"routeStopPairs\"]
                        # Format: \"route1:stop1,route2:stop2\"
                        for pair in pairs_str.split(\",\"):
                            if \":\" in pair:
                                r, s = pair.split(\":\", 1)
                                pairs.append({\"routeId\": r, \"stopId\": s})
                    
                    # Case 2: Explicit list of objects
                    elif \"subscriptions\" in payload:
                        for s in payload[\"subscriptions\"]:
                            if \"routeId\" in s and \"stopId\" in s:
                                pairs.append({\"routeId\": s[\"routeId\"], \"stopId\": s[\"stopId\"]})
                    
                    # Case 3: Single subscription
                    elif \"routeId\" in payload and \"stopId\" in payload:
                        pairs.append({\"routeId\": payload[\"routeId\"], \"stopId\": payload[\"stopId\"]})
...
async def run_server(host: str = \"0.0.0.0\", port: int = 8000, config: Optional[TransitConfig] = None):
    if not config:
        config = TransitConfig.load()
    
    server = TransitServer(config)
    async with websockets.serve(server.register, host, port):
        print(f\"[SERVER] WebSocket server started on ws://{host}:{port}\")
