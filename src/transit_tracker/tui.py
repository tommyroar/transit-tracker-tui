...
def view_config_diff(config: TransitConfig, config_path: str, console: Console):
    ...
    try:
        ...
        mem_content = yaml.safe_dump(
            config.model_dump(exclude_unset=True), sort_keys=False
        )
        ...
        if not diff:
            ...
            diff_text = "".join(diff)
            console.print(
                Panel(Syntax(diff_text, "diff", theme="monokai"), title="Config Diff")
            )
...
def preview_display_format(config: TransitConfig, console: Console):
    ...
    # Render
    table = Table(
        title="Display Format Preview", show_header=False, box=None, padding=(0, 2)
    )
    ...
    console.print(
        Panel(
            Group(
                ...
                Text(
                    f"Variables: {', '.join(f'{{{k}}}' for k in DISPLAY_VARIABLES)}",
                    style="dim",
                ),
                Text(""),
                table,
...
async def diff_profiles_wizard(config: TransitConfig, console: Console):
    ...
    choices = [questionary.Choice(title=os.path.basename(p), value=p) for p in profiles]

    other_path = await questionary.select(
        "Select profile to diff against:", choices=choices
    ).ask_async()
    ...
    try:
        ...
        if not diff:
            rprint(
                "[green]No differences detected between selected profile and in-memory config.[/green]"
            )
        else:
            diff_text = "".join(diff)
            console.print(
                Panel(
                    ...
                    title=f"Diff: {os.path.basename(other_path)} vs Current",
                )
            )
...
def view_service_logs(console: Console):
    ...
    try:
        ...
        console.print(
            Panel(Text(last_lines), title=f"Service Logs (last 50 lines) - {log_path}")
        )
        input("\nPress Enter to continue...")
    except Exception as e:
...
def check_service_status():
    ...
    res = os.system(
        f"launchctl list {PLIST_NAME.replace('.plist', '')} > /dev/null 2>&1"
    )
    if res == 0:
    ...
    try:
        ...
        proc = subprocess.run(
            ["pgrep", "-f", "transit-tracker service"], capture_output=True, text=True
        )
...
async def manage_service_menu(
    ...
    while True:
        ...
        if status == "UNSUPPORTED":
            rprint(
                "[bold red]Background service management is only supported on macOS.[/bold red]"
            )
            break
        ...
        action = await ask_with_live_dashboard(
            ...
            choices=[
                ...
                questionary.Choice(
                    "Restart Service",
                    disabled="Service not running" if "RUNNING" not in status else None,
                ),
                "View Logs",
                "Back",
            ],
...
async def change_panels_wizard(
    config: TransitConfig, config_path: str, console: Console
):
...
async def change_brightness_wizard(
    config: TransitConfig, config_path: str, console: Console
):
    presets = [
    ...
    if val == "custom":
        val = await questionary.text(
            "Enter brightness (0-255):", default=str(config.service.display_brightness)
        ).ask_async()
...
def make_dashboard(config: TransitConfig, config_path: str) -> Panel:
    ...
    if "RUNNING" in status:
        status_text.append(
            f" (PID: {pid}, Uptime: {uptime}, Msg: {messages})", style="dim"
        )
    ...
    panels_text = Text(
        f"Hardware Setup: {config.service.num_panels} Panel(s)", style="magenta"
    )
    config_file_text = Text(
        f"Current Config: {config_path or 'No file loaded (in-memory)'}", style="dim"
    )
    ...
    if client_count > 0:
        ...
        for c in clients:
            ...
            names.append(name)
        client_text = Text(
            f"Proxy Clients ({client_count}): {', '.join(names)}", style="green"
        )
    else:
    ...
    panel_group = Group(
        ...
        else Text("No stops configured yet.", style="italic dim"),
    )

    return Panel(
        panel_group,
        title="[bold cyan]Transit Tracker Manager[/bold cyan]",
        expand=False,
        border_style="cyan",
    )
...
async def ask_with_live_dashboard(
    title, choices, config, config_path, console, default=None
):
    ...
    while True:
        ...
        monitor_task = asyncio.create_task(monitor())
            ...
            [prompt_task, monitor_task], return_when=asyncio.FIRST_COMPLETED
        )
...
async def profile_detail_submenu(config_path: str, console: Console):
    ...
    try:
        while True:
            ...
            table = Table(
                title=f"Profile: {os.path.basename(config_path)}",
            ...
            for sub in config.subscriptions:
                ...
                route_arrivals = [
                    a
                    for a in arrivals
                    if a.get("routeId") == sub.route
                    or a.get("routeName") == sub.route.split(":")[-1]
                ]
            ...
            footer.add_row(f"Config: {config_path}", f"Last Refresh: {last_refresh}")
            console.print(footer)
            console.print("\n")

            action = await questionary.select(
                "Profile Actions:",
                choices=["Refresh", "Activate Profile (Set as Default)", "Back"],
            ).ask_async()
            ...
            if not action or action == "Back":
                ...
                console.print(
                    f"[bold green]✓ Activated {os.path.basename(config_path)}[/bold green]"
                )
                console.print(
                    "[dim]The background service will hot-reload automatically.[/dim]"
                )
                time.sleep(2)
    finally:
        await api.close()
...
async def profiles_menu(console: Console):
    ...
    while True:
        ...
        if not profiles:
            console.print(
                "[yellow]No profile (.yaml) files found in project root or .local/[/yellow]"
            )
            time.sleep(2)
            break
        ...
        selected_path = await questionary.select(
            "Select a Profile to View:", choices=choices
        ).ask_async()
                ...
                if c_action == "Debug":
                    ...
                    f_action = await ask_with_live_dashboard(
                        ...
                        choices=[
                            ...
                            questionary.Choice(
                                "Save Config File",
                                disabled="No config file loaded"
                                if not config_path
                                else None,
                            ),
                            "Save Config File As...",
                    ...
                    if d_action == "Flash Device":
                        selected_port = await questionary.select(
                            "Select your Transit Tracker device:", choices=ports
                        ).ask_async()
                        if selected_port:
                            if not is_bootstrapped(selected_port):
                                console.print(
                                    "[bold yellow]Warning: This device does not appear to have the transit-tracker firmware installed yet.[/bold yellow]"
                                )
                                do_install = await questionary.confirm(
                                    "Do you want to install the base firmware from the official website? (This will erase existing data)"
                                ).ask_async()
                                if do_install:
                                    success = flash_base_firmware(selected_port)
                                    if success:
                                        console.print(
                                            "[green]Base firmware installed. Continuing with device configuration...[/green]"
                                        )
                                        ...
                                        console.print(
                                            "[red]Base firmware installation failed.[/red]"
                                        )
                                else:
                                    console.print(
                                        "Continuing with configuration update anyway..."
                                    )
                                    flash_hardware(selected_port, config)
                            else:
                                flash_hardware(selected_port, config)
                        ...
                        if selected_port:
                            if load_hardware_config(selected_port, config):
                                if config_path:
                                    ...
                                    print(
                                        "Configuration read into memory. Please save it to a file."
                                    )
    ...
    while True:
        ...
        if added or removed:
            ...
            for p in added:
                ...
                rprint(
                    f"[bold green]USB Device Connected:[/bold green] [cyan]{model}[/cyan] at {p}"
                )
            for p in removed:
                model = known_ports[p]
                rprint(
                    f"[bold yellow]USB Device Disconnected:[/bold yellow] [dim]{model}[/dim]"
                )
