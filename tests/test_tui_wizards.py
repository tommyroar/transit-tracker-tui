...
from src.transit_tracker.config import TransitConfig
from src.transit_tracker.tui import (
...
def test_api_mode_wizard_choices(mock_config):
    ...
    with patch("questionary.select") as mock_select, \
        ...
        change_api_mode_wizard(mock_config, "config.yaml")
        ...
        mock_instance.ask.return_value = False
        # When False, it asks for a text URL
        mock_text.return_value.ask.return_value = "wss://test.api"
        
        change_api_mode_wizard(mock_config, "config.yaml")
        args, kwargs = mock_select.call_args
        assert kwargs["default"] is False

def test_panels_wizard_choices(mock_config):
    """Verify that change_panels_wizard uses correct default value format."""
    with patch("questionary.select") as mock_select, \
         patch.object(TransitConfig, "save") as mock_save:
        mock_instance = MagicMock()
        mock_instance.ask.return_value = "3"
        mock_select.return_value = mock_instance
        
        mock_config.num_panels = 2
        change_panels_wizard(mock_config, "config.yaml")
